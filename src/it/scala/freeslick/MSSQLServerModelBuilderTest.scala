// Copied from https://github.com/slick/slick/blob/3.0/slick-testkit/src/main/scala/com/typesafe/slick/testkit/tests/ModelBuilderTest.scala
...
class MSSQLServerModelBuilderTest extends TestkitTest[JdbcTestDB] with Logging {
  ...
  def test: Unit = ifCap(jcap.createModel) {
    ...
    class TypeTest(tag: Tag) extends Table[(String, Boolean, Byte, Short, Int, Long, Float, Double, String, java.sql.Date, java.sql.Time, java.sql.Timestamp, java.sql.Blob //,java.sql.Clob
    , Option[Int], (Option[Boolean], Option[Byte], Option[Short], Option[Int], Option[Long], Option[Float], Option[Double], Option[String], Option[java.sql.Date], Option[java.sql.Time], Option[java.sql.Timestamp], Option[java.sql.Blob] //,Option[java.sql.Clob]
    ))](tag, "TYPE_TEST") {
    ...
    Await.result(db.run(DBIO.seq(ddl.create.named("DDL Create"))), Duration.Inf)
    ...
    assertEquals(model.tables.toString, 5, model.tables.size); {
      ...
      ifCap(jcap.defaultValueMetaData) {
        ...
        def columnDefault(name: String) = column(name)
      ...
      */
      ...
      if (!tdb.profile.toString.contains("OracleDriver")) { // FIXME: we should probably solve this somewhat cleaner
        //TODO Sue should be using the real date and time types
        assertEquals("String", column("java_sql_Date").tpe)
        assertEquals("String", column("Option_java_sql_Date").tpe)
        assertEquals("String", column("java_sql_Time").tpe)
        assertEquals("String", column("Option_java_sql_Time").tpe)
      ...
      ifCap(jcap.defaultValueMetaData) {
        ...
        Seq(
          (1, noDefaultTest.map(_.int) += 2),
          (None, noDefaultTest.map(_.stringOption).result.head),
          (2, noDefaultTest.map(_.int).result.head)
        ).foreach { case (v, s) => assertEquals(v, Await.result(db.run(s), Duration.Inf)) }
      }
    }
  }
}
